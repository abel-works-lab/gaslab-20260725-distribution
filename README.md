...
（自動でブラウザが開かない場合は、ターミナルに表示されているURLをコピーして手動で開いてください。既に一度ログイン済みのPCでは、確認なしでそのまま処理が進むこともあります）

ここまで進むと、データの投入まで自動で完了します。セットアップ完了時に、自動でVercelの新規プロジェクト作成画面が開きます（開かない場合は次の手順6で手動でアクセスしてください）。
...
5. 「Deploy」を押す
6. デプロイ完了後、Vercelのプロジェクト画面（Overview）を開く。画面上部の「Production Deployment」というカードの中に、次の2つのURLらしき項目が並んで表示されている
   - **Deployment**：`https://〇〇-xxxxxxxx-プロジェクト名.vercel.app`のような、ランダムな文字列を含む長いURL → **これは使わない**
...
- **`WORKOS_REDIRECT_URI`をなぜ変える必要があるのか**：ログイン後にWorkOSが「どこに戻すか」は、事前に登録したURLでしか許可されません。`.env.local`には手順2で`http://localhost:3100/callback`が自動設定されていますが、Vercel上ではアプリが別の場所（Vercelのサーバー）で動くため、戻り先のURLも変える必要があります。
