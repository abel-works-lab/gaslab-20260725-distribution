// data/medical_cost_r5_00.json を Convex の medicalCost に投入。
// 使い方: webapp/ で `node --use-system-ca scripts/seed_medical_cost.mjs`
// seedMedicalCost は internalMutation（外部クライアントから呼べない）のため、管理者キーが必要。
...
const rows = JSON.parse(
  fs.readFileSync(path.join(root, 'data', 'medical_cost_r5_00.json'), 'utf-8'),
)
...
const res = await client.mutation(internal.medicalCost.seedMedicalCost, { rows })
console.log('seeded medicalCost:', res)
